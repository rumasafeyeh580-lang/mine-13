...
from modules.bg_removers.birefnet_model.config import Config
from modules.bg_removers.birefnet_model.dataset import class_labels_TR_sorted
from modules.bg_removers.birefnet_model.backbones.build_backbone import build_backbone
from modules.bg_removers.birefnet_model.refinement.stem_layer import StemLayer
from modules.bg_removers.birefnet_model.modules.decoder_blocks import BasicDecBlk, ResBlk
from modules.bg_removers.birefnet_model.modules.lateral_blocks import BasicLatBlk


def image2patches(image, grid_h=2, grid_w=2, patch_ref=None, transformation='b c (hg h) (wg w) -> (b hg wg) c h w'):
    if patch_ref is not None:
        # Use ceil to ensure the grid fully covers the image even when not divisible
        grid_h = math.ceil(image.shape[-2] / patch_ref.shape[-2])
        grid_w = math.ceil(image.shape[-1] / patch_ref.shape[-1])
    # Pad on right/bottom so H and W are divisible by grid_h/grid_w
