...
async def generate_from_base64(request: GenerateRequest) -> GenerateResponse:
    ...
    try:
        ...
        if not result.ply_file_base64:
            raise ValueError("PLY generation failed")

        # Ensure ply_file_base64 is bytes
        ply_bytes = result.ply_file_base64 if isinstance(result.ply_file_base64, bytes) else result.ply_file_base64.encode()

        # Compress the ply file if compression is enabled
        if settings.output.compression:
            ply_bytes = pyspz.compress(ply_bytes, workers=1)
            logger.info(f"Compressed PLY size: {len(ply_bytes)} bytes")

        # Encode to base64 string for JSON response
        result.ply_file_base64 = base64.b64encode(ply_bytes).decode("utf-8")
